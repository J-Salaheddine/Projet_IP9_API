package com.projet.outils;

public interface StaticValues {

	public static int NOMBRE_DE_DIVISION_RG = 8;
	public static int NOMBRE_DE_DIVISION_BY = 16;
	public static int NOMBRE_DE_DIVISION_WB = 16;
	
	public static int INTERVALE_RG = 255;
	public static int INTERVALE_BY = 510;
	public static int INTERVALE_WB = 768;
	
	public static String BDD_SIGANTURE = "BDD\\Signature\\";
	public static String BDD_IMAGES = "BDD\\Image\\";
	public static String BDD_RES = "BDD\\res\\";
	
}
